...
async def get_student_info(email: str = "", phone: str = "", contact_id: str = "") -> str:
    ...
    """
    # 🔒 Guard compartido con la tool de cobranzas: solo identidades verificadas
    # por el canal (sesión en el sitio o número de WhatsApp) ven datos de cuenta.
    from utils.identity_guard import bloquear_si_no_verificado

    if rechazo := await bloquear_si_no_verificado("get_student_info", email):
        return rechazo
