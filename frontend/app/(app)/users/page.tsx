...
/**
 * /users — gestión del equipo (CRUD).
 *
 * Supervisores listan y editan colas. Admins además crean y borran usuarios
 * (enforced por backend en `/auth/users`).
 *
 * Hasta acá el CRUD vivía dentro de `/settings/page.tsx`; lo movimos a una
 * ruta propia para que el nav rail tenga un ícono de "Equipo" y no haya que
 * pasar por Configuración para llegar.
 */
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      <div className="px-6 py-4 border-b border-border">
        ...
        <p className="text-xs text-fg-dim mt-0.5">
          Usuarios del workspace: agentes, supervisores y admins.
        </p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto scroll-thin p-6 space-y-6 max-w-3xl">
        <section>
              ...
              <h2 className="text-sm font-semibold flex items-center gap-2">
                ...
                {!isAdmin && (
                  <span className="text-[10px] font-normal text-fg-dim flex items-center gap-1">
                    <Shield className="w-3 h-3" /> supervisor: solo colas — crear/borrar requiere admin
                  </span>
                )}
              </h2>
              <p className="text-[11px] text-fg-dim">
                Crear un usuario lo agrega a Supabase Auth + profile y aparece automáticamente en
                el dropdown de "Asignar a" en el inbox.
              </p>
            </div>
            {isAdmin && (
          ...
          <div className="space-y-1.5">
            {usersQ.isLoading && <div className="text-xs text-fg-dim">Cargando equipo…</div>}
            {usersQ.error && (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[11px] text-fg-dim truncate">
                      ...
                      {u.queues?.length > 0 && (
                        <span className="ml-2">
                          · {u.queues.length} cola{u.queues.length === 1 ? "" : "s"}
                        </span>
                      )}
                    </div>
                  </div>
